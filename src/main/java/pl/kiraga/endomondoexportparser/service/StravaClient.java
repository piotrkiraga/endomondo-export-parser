 ...
 * see {@link StravaUploadResultDto}.
 */
@Service
public class StravaClient {
    ...
    String accessToken() {
        StravaTokensDto current = tokenStore.load()
    ...
    public StravaTokensDto refreshAccessToken(String refreshToken) {
     ...
     * object on this grant type, harmlessly ignored by {@link StravaTokensDto}.
     */
    public StravaTokensDto exchangeAuthorizationCode(String code) {
        return exchangeForTokens("authorization_code", "code", code);
    }

    private StravaTokensDto exchangeForTokens(String grantType, String grantParamName, String grantParamValue) {

        requireCredentials();

        StravaTokensDto tokens = withRetryOn429(() -> restClient.post()
    ...
    public StravaUploadResultDto uploadTcx(Path tcxFile, String externalId, String name, String description) {
        ...
        return withRetryOn429(() -> restClient.post()
                ...
                .body(parts)
                .retrieve()
                .body(StravaUploadResultDto.class));

    }

    /**
     * Checks an upload's processing status. Poll until {@link StravaUploadResultDto#hasActivityId()}
     * (done — whether a fresh upload or Strava's duplicate rejection, both count as success
     * per design.md) or {@link StravaUploadResultDto#failed()}.
     */
    public StravaUploadResultDto checkUploadStatus(long uploadId) {
    ...
    public StravaActivityDto createManualActivity(String name, String sportType, Instant startDateLocal,
    ...
    public StravaActivityDto updateActivity(long activityId, String name, String sportType, String description) {
     ...
     * {@link StravaActivityDto#gearId}, which a create/update call's own response echoes
     ...
     * has on file.
     */
    public StravaActivityDto getActivity(long activityId) {
        return withRetryOn429(() -> restClient.get()
                .uri("/api/v3/activities/{id}", activityId)
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken())
                .retrieve()
    ...
    public StravaGearDto getGear(String gearId) {
        return withRetryOn429(() -> restClient.get()
                .uri("/api/v3/gear/{id}", gearId)
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken())
                .retrieve()
                .body(StravaGearDto.class));
    }

    /**
     * The connected athlete's profile, including every bike and shoe with its name —
     * the one call {@link StravaDictionaryService#refresh} uses to populate the whole gear
