# photo-report-copy-path Specification

## Purpose
Gives the photo report a one-click "copy path" control per photo, so a photo's absolute local file path can be pasted straight into Strava's manual attach dialog without locating the file in a file browser — closing the loop Strava's API leaves open, since it has no endpoint to upload a photo at all.
