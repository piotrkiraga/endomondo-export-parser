...
This capability documents the parser's **current, observed** behavior — defects included — as pinned by characterization tests. Requirements marked "(pinned defect)" describe behavior that is wrong on purpose here; the parser-fix change is expected to MODIFY them.
